...
declare global {
  namespace jest {
    type TypeOfMatcher<T extends jest.CustomMatcher, R> = (...args: SecondParameter<T>) => R
    interface Matchers<R> {
...
function toLeastEqual(this: jest.MatcherContext, ex: any, toBe: any): jest.CustomMatcherResult {
...
function* __(obj: any, path: string = ""): any {
...
function toMatchHttpError(this: jest.MatcherContext, error: unknown, to: HttpError<any>) {
  const pass = { pass: true, message: () => 'function threw an error' }
  if (!error) {
...
function toCatch(this: jest.MatcherContext, expectCB: unknown, afterThrowingCB: (e: Error) => void) {
  if (!(typeof expectCB === 'function')) {
    return { pass: false, message: () => 'fn in `expect(fn).afterThrowing(...)` is not of type function' }
  }
  if (!(typeof afterThrowingCB === 'function') || afterThrowingCB.length !== 1) {
    return { pass: false, message: () => 'fn in `expect(...).afterThrowing(fn)` is not of type function' }
  }

  let toCatch_: boolean = false
  try {
    expectCB()
  } catch (error) {
  ...
  return toCatch_ ?
    ...
    { pass: false, message: () => 'function didn`t throw any error' }
}
