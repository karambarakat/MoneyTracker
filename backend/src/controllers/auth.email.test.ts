...
describe('auth email', () => {
  ...
  describe('/auth/local/register', () => {
    ...
    test('2xx: optional fields omitted', async () => {
      ...
      expect(res.body.data).toLeastEqual({ email: 'auth@example.com' })
    })

    test('2xx: all fields', async () => {
      const token = 'Basic ' + Buffer.from(
        ['e2@example.com', 'password']
          .join(':')).toString('base64')
      ...
      expect(res.body.data).toBeInTheShapeOf(Null(shape, [], ['picture']))
      expect(res.body.data).toLeastEqual({ email: 'e2@example.com', displayName: 'Karam' })
    })

    test('2xx: type checks', async () => {
      const token = 'Basic ' + Buffer.from(
        ['e3@example.com', 'password']
          .join(':')).toString('base64')
      ...
      expect(res.body.data).toBeInTheShapeOf(Null(shape, [], ['picture']))
      expect(res.body.data).toLeastEqual({ email: 'e3@example.com', displayName: '1234' })
    })
    ...
    test('400', async () => {
      ...
      const expectedError = ValidationError({ msg: '', errors: { email: '' } })
