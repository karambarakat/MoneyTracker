...
import { FieldsRequired, ResourceWasNotFound, TokenFailed } from "@httpErrors/errTypes"
...
describe('log', () => {
  ...
  beforeAll(async () => {
    ...
    data.second = (await request(app)
      .post('/api/v1/auth/local/register')
      .set('Authorization', 'Basic ' + Buffer.from(
        ['e2log@example.com', 'password']
          .join(':')).toString('base64'))
      .send()).body.data

    data.category = (await request(app)
  ...
  describe('/log post', () => {
    ...
    test('400', async () => {
      ...
      expect(res.body.error).toMatchHttpError(FieldsRequired(['title', 'amount']))
  ...
  describe('/log/:id put', () => {
    ...
    test('2xx', async () => {
      const res = await req(data.logs[1]._id)
        .set('Authorization', 'Bearer ' + data.first.token)
        .send({ title: 'put request', amount: 99, note: 'put request' })

      expect(res.body.data).toBeInTheShapeOf(shape)
      expect(res.body.data).toLeastEqual({ title: 'put request', amount: 99, note: 'put request' })
    })
  })
