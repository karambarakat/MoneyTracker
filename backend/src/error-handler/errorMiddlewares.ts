import { UnknownServerError, ResourceWasNotFound, ValidationError, BadJsonPayload } from '@error/HttpErrors'
import throwHttpError from '@error/throwHttpError'
import { CustomError } from '@myTypes/HTTPError'
...
export function e400_MongooseValidation(
    ...
    if (err.name === 'ValidationError' && err.errors) {
        //@ts-ignore
        const myErr = prepareValidationError(err._message, Object.keys(err.errors).reduce((acc: { [key: string]: string }, key) => {
