import { EmailOrPasswordIncorrect, QuickValidationError, ServerError, UserAlreadyExist } from "@error/HttpErrors"
import throwHttpError, { throwQuickHttpError } from "@error/throwHttpError"
import User from "@models/User"
import { NextFunction, Request, Response } from "express"
import { ExtractJwt, Strategy as jwtStrategy } from 'passport-jwt'
import { jwt_payload } from '@myTypes/jwt'
...
async function local_register(req: Request, res: Response, next: NextFunction) {
    ...
    const newUser = await User.create(truthy({
 ...
 *   @access  Private
 */
async function local_change_password(req: Request, res: Response, next: NextFunction) {
 ...
 *   @access  Private
 */
async function local_set_password(req: Request, res: Response, next: NextFunction) {
    if (!req.user) throw PrivateRouteError
...
async function local_info(req: Request, res: Response, next: NextFunction) {
    ...
    else if (user.provider.some(e => e === 'local') && user.password) res.send('login')
